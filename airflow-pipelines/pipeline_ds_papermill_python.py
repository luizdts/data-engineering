...
def notebook_exec(**kwargs):
   ...
   parameters = dict( test_size_ = 0.25
   					, dataset = dags_path+"projeto-data-science-papermill/data/ecomerce.csv"
   					, model = dags_path+"projeto-data-science-papermill/model/"
   					, n_estimators = 1000
   					, max_depth = 5
   				)
)
...
with DAG(dag_id="pipeline_data_science_papermill_python",
    ...
    op_kwargs={   
                'input_notebook': dags_path+'projeto-data-science-papermill/regression-project.ipynb'
               ,'output': dags_path+'projeto-data-science-papermill/output-{{ execution_date }}.ipynb'
            }
    )
